...
void program(){
    ...
    while(fgets(word,LENGTH,file) != NULL){
        ...
        strcpy(movieName[lineIndex].directors,temp2);*/
        ...
        i=0;
        const char *delp2=strtok(temp2,",");
        movieName[lineIndex].directors = (char**)malloc(sizeof(char*)*LENGTH);
        while(delp2 != NULL){
            movieName[lineIndex].directors[i] = (char*)malloc(sizeof(char)*LENGTH);
            strcpy(movieName[lineIndex].directors[i],delp2);
            delp2=strtok(NULL,",");
            i++;
        }
...
void information_of_movie(struct Budget *budget,struct Name *name){
    ...
    char *nameOfMovie = (char*)malloc(sizeof(char)*LENGTH);
    printf("\nEnter movie name : ");
    scanf(" %[^\n]",nameOfMovie);
    int index;
...
void print_movie_information(struct Budget *budget,struct Name *name,int index){
    ...
    printf("Genres : ");
    while(name[index].genre[i]!=NULL){
        printf("%s ",name[index].genre[i]);
        i++;}
    printf("\nDirector : ");
    i=0;
    while(name[index].directors[i]!=NULL){
...
void print_genres(struct Name *name) {
    ...
    for (i = 0; i < SIZE_OF_FILE; i++) {
        int genreIndex = 0;
        char **genres = (char **)malloc(SIZE_OF_FILE * sizeof(char *));
        genres = name[i].genre;
